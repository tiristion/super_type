		...
		override public function handleNotification(notification:INotification):void {
			switch(notification.getName()) {
				case GeneralNotifications.USER_DATA_UPDATED:
					(viewLogic as TopPanelViewLogic).userDataUpdate(notification.getBody().name as String,notification.getBody().score as Number);
					break;
				...
				case GeneralNotifications.UPDATE_LEVEL_SCORE:
					(viewLogic as TopPanelViewLogic).updateLevelScore(notification.getBody().levelScore as Number,notification.getBody().mistakes as Number);
					break;
